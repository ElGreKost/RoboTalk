...
from torchrl.modules import DuelingCnnDQNet, EGreedyModule, QValueActor, MLP, Actor
...
def make_env(
    ...
    drone_env = Drone()
    base_env = DroneWrapper(drone_env)
    base_env.set_seed(0)

    env = TransformedEnv(
        base_env,
        Compose(
            ObservationNorm(in_keys=["observation"]),  # todo useful? how to pass the norm_sd
            StepCounter(),
            # RewardScaling(loc=0.0, scale=0.1), todo useful?
            # CatFrames(4, in_keys=["pixels"], dim=-3), todo useful? For velocity through state
        ),
    )
    return env
...
def make_model(dummy_env):
    ...
    actor = QValueActor(value_net, spec=env.action_spec)
    tensordict = dummy_env.fake_tensordict()
    actor(tensordict)  # do we truly need it, or is it automatically inferred?
    ...
    actor_explore = Seq(actor, exploration_module)
...
def get_collector(
    ...
    env_arg = [make_env()] * num_collectors
    data_collector = cls(
        env_arg,
...
def get_loss_module(actor, gamma):
    ...
    target_updater = SoftUpdate(loss_module, eps=0.995)     # todo what is SoftUpdate
...
# The following setting should work well in practice, and the performance of
# the algorithm should hopefully not be too sensitive to slight variations of these.
...
actor, actor_explore = make_model(env)

loss_module, target_net_updater = get_loss_module(actor, gamma)

stats = env.transform[0].state_dict()
...
collector = SyncDataCollector(
    ...
    init_random_frames=1024    # todo useful?`
)
...
# reset = env.reset()
# dict_items([('observation', tensor([-2.8460,  0.0000, -0.3162])),
# ('action', tensor([False, False, False, False, False,  True])), ('done', tensor([False])),
# ('terminated', tensor([False])), ('step_count', tensor([0]))])
#
# Get model
# actor, actor_explore = make_model(env)
