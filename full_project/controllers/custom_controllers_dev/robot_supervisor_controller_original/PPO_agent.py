...
class PPOAgent:
    ...
    def train_step(self, batch_size=None):
        ...
        :return: None
        """
        # Default behavior waits for buffer to collect at least one batch_size of transitions
        ...
        for _ in range(self.ppo_update_iters):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))), batch_size, False):
                ...
                action_loss.backward()  # Perform a backward step to compute new gradients
                nn.utils.clip_grad_norm_(self.actor_net.parameters(), self.max_grad_norm)  # Clip gradients
                self.actor_optimizer.step()  # Perform a training step based on gradients
